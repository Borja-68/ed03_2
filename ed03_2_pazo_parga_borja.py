"""Module providing a impots functions."""
import math
import logging
import sys
...
def main():
    ...
    try:
        ...
        print(f"Suma de {x} + {y} = {calc.sumar(x, y)}")
        print(f"Resta de {x} - {y} = {calc.restar(x, y)}")
        print(f"Multiplicación de {x} * {y} = {calc.multiplicar(x, y)}")
        print(f"División de {x} / {y} = {calc.dividir(x, y)}")
        # Ejemplos de uso de operaciones avanzadas
        print("\n=== Operaciones Avanzadas ===")
        print(f"Potencia de {x}^{y} = {calc.potencia(x, y)}")
        print(f"Raíz cuadrada de {x} = {calc.raiz_cuadrada(x)}")
        print(f"Logaritmo natural de {x} = {calc.logaritmo_natural(x)}")
        print(f"Logaritmo base 10 de {x} = {calc.logaritmo_base_10(x)}")
